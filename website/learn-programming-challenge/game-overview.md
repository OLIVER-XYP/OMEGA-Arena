...
Players begin play with a shipyard, and can use collected halite to build new ships. To efficiently collect halite, players must devise a strategy based on building ships and dropoff points. Ships can explore the ocean, collect halite, and store it in the shipyard or in dropoff points.
<br/><br/>
...
Ships interact directly in two ways:
- If multiple ships occupy the same location after movement, they **collide** and each ship loses **50 HP**. Ships with HP less than or equal to 0 are destroyed and drop cargo to that cell.
- If there are two or more ships belonging to any opponent within a four-cell radius of your ship, your ship is **inspired** by the competition. An inspired ship collects halite from the sea at the normal rate, but receives an additional 200% bonus.
...
This branch runs with ruleset identifier: `halite3-hp-collision-v1`.
<br/><br/>
...
Optional Combat Commands (when `ENABLE_COMBAT_COMMANDS=true`) | `a` attack ship, `x` attack structure, `d` defend, `h` heal (no-op in current ruleset)

<br/><br/>

##### Win Conditions
The game continues for a ruleset-defined number of turns (this codebase defaults to up to 1000 turns depending on map size). The winner is determined by the **sum of halite stored in all owned structures** (shipyard + all dropoffs) at game end. Ships do not need to return on the final turn to satisfy a special endgame rule; standard deposit rules still apply when on owned structures.
<br/><br/>

##### Note on Game Rules Changes
The ruleset is versioned and configurable in engine constants. Keep bot/runtime/website aligned to the same ruleset version for reproducible results.
