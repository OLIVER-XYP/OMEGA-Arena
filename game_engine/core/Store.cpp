 ...
 * Obtain a new player.
 *
 * @param factory The player's factory location.
 * @param command A label for the player command/source.
 * @return The new player.
 */
Player &Store::new_player(Location factory, const std::string &command) {
    auto player = player_factory.make(factory, command);
    return players.emplace(player.id, player).first->second;
}
