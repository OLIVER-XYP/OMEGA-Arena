...
std::string backend_failure_message(const GameConfig &config) {
    return std::string("Requested engine backend '") +
           engine_backend_mode_name(config.runtime.engine_backend) +
           "' is unavailable: " + gpu::cuda_turn_backend_unavailable_reason() +
           " (" + gpu::cuda_turn_backend_capability_summary() + ")";
}
...
void HaliteImpl::process_turn(TaskExecutor &executor, TurnExecutionProfile *profile) {
    ...
    auto step_result = backend->step(state, commands, sink, executor, profile);
    observers::TurnStatsCollector turn_stats_collector;
    turn_stats_collector.collect(state, game.get_config(), executor);
 ...
 */
ITurnBackend *HaliteImpl::selected_backend() {
    const auto mode = game.get_config().runtime.engine_backend;
    if (mode == EngineBackendMode::Cpu) {
        return &cpu_backend_;
    }
    if (gpu::cuda_turn_backend_available()) {
        if (!gpu_backend_) {
            gpu_backend_ = gpu::make_cuda_turn_backend(turn_engine_, game.get_config());
        }
        if (gpu_backend_) {
            return gpu_backend_.get();
        }
    }
    if (mode == EngineBackendMode::Auto) {
        return &cpu_backend_;
    }
    throw std::runtime_error(backend_failure_message(game.get_config()));
}

HaliteImpl::HaliteImpl(Halite &game) : game(game), turn_engine_(game.get_config()), cpu_backend_(turn_engine_) {}
