...
const char *engine_backend_mode_name(EngineBackendMode mode) {
    switch (mode) {
        case EngineBackendMode::Cpu:
            return "cpu";
        case EngineBackendMode::Gpu:
            return "gpu";
        case EngineBackendMode::Auto:
            return "auto";
    }
    return "cpu";
}

EngineBackendMode parse_engine_backend_mode(const std::string &value) {
    if (value == "cpu") {
        return EngineBackendMode::Cpu;
    }
    if (value == "gpu") {
        return EngineBackendMode::Gpu;
    }
    if (value == "auto") {
        return EngineBackendMode::Auto;
    }
    throw std::invalid_argument("engine backend must be one of: cpu, gpu, auto");
}
