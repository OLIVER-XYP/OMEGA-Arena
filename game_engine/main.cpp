...
int main(int argc, char *argv[]) {
    ...
    ValueArg<std::string> engine_backend_arg("", "engine-backend", "Turn simulation backend: cpu, gpu, or auto.", false,
                                             "cpu", "cpu|gpu|auto", cmd);
    ValueArg<unsigned long> gpu_batch_size_arg("", "gpu-batch-size", "GPU batch size for batch-capable runners.", false,
                                               1, "positive integer", cmd);
    ...
    try {
        game_config.runtime.engine_backend = hlt::parse_engine_backend_mode(engine_backend_arg.getValue());
    } catch (const std::invalid_argument &err) {
        std::cerr << err.what() << std::endl;
        return 1;
    }
    game_config.runtime.gpu_batch_size = gpu_batch_size_arg.getValue();
    if (game_config.runtime.gpu_batch_size == 0) {
        std::cerr << "gpu batch size must be positive" << std::endl;
        return 1;
    }
