...
# ── 策略参数 ──────────────────────────────────────────────────────────────────
STOP_RATIO       = 0.04
SPAWN_END_RATIO  = 0.55
SAFE_MARGIN      = 1
RUSH_TURNS       = 20
MIN_RETURN_CARGO = 150
DROPOFF_BUFFER   = 500
DROPOFF_BUFFER2  = 250
MAX_DROPOFFS     = 2
...
# 战略占领参数
STRATEGIC_DIST       = 8     # 超过此距离才触发战略路径
STRATEGIC_RICH       = 200   # 格子 halite 须达到此值才算"富矿"
STRATEGIC_EFF_THRESH = 18.0  # 效率得分低于此时才考虑战略目标（避免放弃好的本地矿）

FALLBACK_MIN_HALITE = 50   # 残矿低于此值时不追，直接回家，避免边缘无效徘徊

# 攻击参数
ENABLE_ATTACK = True   # 是否启用主动拦截逻辑
ATTACK_RATIO  = 1.5    # 敌船 halite / 自船 halite > 此值时发动攻击（碰撞合算）

# 终局参数（1000 轮游戏）
END_RUSH_TURNS  = 20   # 剩余回合 <= 此值时，所有带货船强制返回
RETURN_BUFFER   = 6    # 强制返回 buffer（turns_left <= home_dist + RETURN_BUFFER）
...
def find_richest_cell(gmap):
    """全图扫描，返回 halite 最多的未占用格子位置。"""
    best_pos, best_hal = None, 0
    for y in range(gmap.height):
        for x in range(gmap.width):
            pos = Position(x, y)
            h = gmap[pos].halite_amount
            if h > best_hal:
                best_hal, best_pos = h, pos
    return best_pos
...
def handle_no_target(gmap, ship, drops, enemy_drops, insp_map, claimed,
    ...
    # 如果 fallback 格子 halite 太少（残矿），不值得追，直接迁移到全图最富矿区
    if fallback is not None and gmap[fallback].halite_amount < FALLBACK_MIN_HALITE:
        fallback = None
    if fallback is None:
        richest = find_richest_cell(gmap)
        if richest is not None and gmap[richest].halite_amount >= FALLBACK_MIN_HALITE:
            fallback = richest
...
while True:
    ...
    # ── 按优先级排序：返程船先处理，且距基地近的先处理防止堵塞 dropoff 入口 ──
    def ship_key(s):
        m = ship_mode.get(s.id, 'collect')
        d = gmap.calculate_distance(s.position, nearest_drop(gmap, s.position, drops))
        if m == 'return':         return (0, d, s.id)
        if s.id in dropoff_ships: return (1, d, s.id)
        return                           (2, d, s.id)
