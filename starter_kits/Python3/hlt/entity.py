...
class Ship(Entity):
    ...
    def __init__(self, owner, id, position, halite_amount, hp):
    ...
    def stay_still(self):
        ...
        return "{} {}".format(commands.STAY_STILL, self.id)

    def attack(self, target):
        """Attack an adjacent enemy ship. target must be a Ship instance."""
        return "{} {} {}".format(commands.ATTACK, self.id, target.id)

    def attack_structure(self, owner_id, position):
        """Attack an adjacent enemy structure (shipyard/dropoff)."""
        return "{} {} {} {} {}".format(
            commands.ATTACK_STRUCTURE,
            self.id,
            owner_id,
            position.x,
            position.y,
    ...
    def defend(self):
        """Enter defensive stance for this turn (blocks incoming attacks)."""
        return "{} {}".format(commands.DEFEND, self.id)

    def heal(self):
        """Recover HP in place (mutually exclusive with move/attack/defend)."""
        return "{} {}".format(commands.HEAL, self.id)

    @staticmethod
    def _generate(player_id):
        """
        Creates an instance of a ship for a given player given the engine's input.
        Engine format: ship_id x y halite hp
        """
        ship_id, x_position, y_position, halite, hp = map(int, read_input().split())
        ...
        if ship_id in Ship.__ships.keys():
            ...
            new_ship = Ship(player_id, ship_id, Position(x_position, y_position), halite, hp)
            Ship.__ships[ship_id] = new_ship
            return ship_id, new_ship

    def __repr__(self):
        return "{}(id={}, {}, cargo={} halite, hp={})".format(
            self.__class__.__name__, self.id, self.position, self.halite_amount, self.hp)
