#!/usr/bin/env python3
import argparse, hashlib, json, random, shutil, statistics, subprocess, threading, time, tempfile
...
class TrainingContext:
    def __init__(self, config: Dict):
        ...
        self.game_timeout_sec = float(config.get("game_timeout_sec", max(30, self.turn_limit * 1.5)))
...
def archive_error_logs(ctx: TrainingContext, tag: str) -> List[str]:
    ...
    with ERROR_LOG_LOCK:
        for p in ctx.root.glob("errorlog-*.log"):
            t = ctx.err_dir / f"{safe_tag}_{time.time_ns()}_{p.name}"
            try:
                p.replace(t)
                moved.append(str(t))
            except Exception:
...
def write_attempt_logs(log_dir: Path, game_id: str, attempt: int, cmd: List[str], rc: int, stdout: str, stderr: str):
    ...
    (base.with_suffix(".cmd.txt")).write_text(subprocess.list2cmdline([str(x) for x in cmd]), encoding="utf-8")
    ...
    (base.with_suffix(".meta.json")).write_text(json.dumps({"return_code": rc, "cmd": [str(x) for x in cmd]}, indent=2), encoding="utf-8")
...
def run_one_game(ctx: TrainingContext, params_path: Path, map_size: int, seed_a: int, opponent_cmd: str, retry: int = 2, log_dir: Path = None, game_id: str = "game", swap_positions: bool = False) -> Dict:
    ...
    for attempt in range(retry + 1):
        try:
            p = subprocess.run(cmd, cwd=str(ctx.root), capture_output=True, text=True, timeout=ctx.game_timeout_sec)
            ...
            write_attempt_logs(log_dir, game_id, attempt, cmd, -1, stdout, stderr)
            if attempt == retry:
                return {"ok": False, "reason": "timeout", "timeout_sec": ctx.game_timeout_sec, "error_logs": archive_error_logs(ctx, f"{game_id}_timeout_a{attempt}")}
            continue
        except Exception as e:
            write_attempt_logs(log_dir, game_id, attempt, cmd, -2, "", repr(e))
            if attempt == retry:
                return {"ok": False, "reason": "subprocess_exception", "exception": repr(e), "error_logs": archive_error_logs(ctx, f"{game_id}_exception_a{attempt}")}
            continue
...
def require_parameter_groups(groups: Dict, names: List[str]) -> None:
    missing = [name for name in names if name not in groups]
    if missing:
        raise SystemExit(f"Missing parameter group(s) in config: {', '.join(missing)}")


def choose_base(candidates: List[TrialResult], fallback: Dict[str, str]) -> Dict[str, str]:
    if not candidates:
        return fallback
    return random.choice(candidates).details["params"]


def reset_trial_dir(ctx: TrainingContext, trial_dir: Path) -> None:
    trial_dir = trial_dir.resolve()
    out_dir = ctx.out_dir.resolve()
    if trial_dir.exists() and out_dir in trial_dir.parents:
...
def run_phase_parallel(ctx: TrainingContext, phase_name: str, count: int, base_picker: Callable[[int], Dict[str, str]], groups, games_per_trial: int, flip: float, rw: RewardWeights, min_valid: int, max_workers: int) -> List[TrialResult]:
...
def main():
    ...
    print(f"[setup] bot={ctx.bot_exe} engine={ctx.engine_exe} engine_cfg={ctx.engine_config} max_workers={max_workers} game_timeout_sec={ctx.game_timeout_sec}")
    ...
    required_groups = ["important", "medium", "minor"]
    require_parameter_groups(groups, required_groups)
    ...
    summary = {
        ...
        "game_timeout_sec": ctx.game_timeout_sec,
        "pilot": pilot,
    ...
    if a.mode == "global":
        ...
        phase1_pool = b1 or [TrialResult(score=-1e9, details={"params": base})]
        summary["phases"]["phase1"] = {"best": [x.details for x in b1]}

        p2 = run_phase_parallel(ctx, "phase2", a.phase2, lambda _i: choose_base(phase1_pool, base), [groups["important"], groups["medium"]], a.games, 0.03, rw, min_valid, max_workers)
        b2 = topk(p2, max(4, a.phase2 // 5)) if p2 else phase1_pool
        phase2_pool = b2 or phase1_pool
        summary["phases"]["phase2"] = {"best": [x.details for x in b2]}

        p3 = run_phase_parallel(ctx, "phase3", a.phase3, lambda _i: choose_base(phase2_pool, base), [groups["medium"], groups["minor"]], a.games, 0.01, rw, min_valid, max_workers)
        b3 = topk(p3, max(5, a.phase3 // 4)) if p3 else phase2_pool
        final_pool = b3 or phase2_pool or phase1_pool
        summary["phases"]["phase3"] = {"best": [x.details for x in b3]}
        write_params(ctx.out_dir / "best_params.txt", final_pool[0].details["params"])
